...
use super::helpers::{
    extract_import_name, is_commented_out_code, is_special_function, truncate_string,
};
                        ...
                        for i in line_num..lines.len().min(line_num + 20) {
                            ...
                            if colon_count > 0 && i > line_num {
                                let next_line_indented =
                                    check_line.starts_with(' ') || check_line.starts_with('\t');
...
"#;
    ...
    fn test_edge_case_very_long_function_name() {
        ...
        assert!(
            items
                .iter()
                .any(|i| i.name.contains("very_long_function_name"))
        );
    }
