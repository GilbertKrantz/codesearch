...
pub use maintainability::{CodeChurn, MaintainabilityMetrics};
...
pub fn analyze_file_metrics(path: &Path) -> Result<FileMetrics, Box<dyn std::error::Error>> {
    ...
    let maintainability =
        maintainability::calculate_maintainability_metrics(&complexity, &size, &content, ext);
...
fn calculate_totals(files: &[FileMetrics]) -> TotalMetrics {
    ...
    let avg_maintainability = if total_files > 0 {
        files
            .iter()
            .map(|f| f.maintainability.maintainability_index)
            .sum::<f64>()
            / total_files as f64
    } else {
        0.0
    ...
    let avg_halstead_volume = if total_files > 0 {
        files
            .iter()
            .map(|f| f.complexity.halstead.volume)
            .sum::<f64>()
            / total_files as f64
    } else {
        0.0
...
pub fn print_metrics_report(metrics: &ProjectMetrics, detailed: bool) {
    ...
    println!(
        ...
        metrics.totals.avg_cyclomatic
    );
    println!(
        "  Avg Halstead Volume: {:.2}",
        metrics.totals.avg_halstead_volume
    );
    println!(
        "  Estimated Bugs (Halstead): {:.2}",
        metrics.totals.total_estimated_bugs
    );

    println!("\n{}", "Maintainability:".green().bold());
    println!(
        "  Avg Maintainability Index: {:.2}/100",
        metrics.totals.avg_maintainability
    );
    ...
    if detailed {
        ...
        sorted_files.sort_by(|a, b| {
            b.complexity
                ...
                .cmp(&a.complexity.cyclomatic_complexity)
        });
        ...
        for file in sorted_files.iter().take(20) {
            ...
            println!(
                "    MI: {:.2}/100",
                file.maintainability.maintainability_index
            );
